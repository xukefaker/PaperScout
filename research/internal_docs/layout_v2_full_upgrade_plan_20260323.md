# PaperSearchAgent Layout V2 全量升级计划
...
- 状态：`in_progress`
- 说明：当前正在远端执行 `paper-search-agent build-index`。日志显示 paper 级结构抽取已完成，正在执行 dense encoding。只有当 `index_state.json` 成功写入并且 search 可正常运行后，这一步才算完成。
...
- 状态：`done`
- 说明：`offline-run / offline-pause / offline-status` 已通过 `paper-search-agent` CLI 暴露，项目根目录的 `offline.sh` 已作为唯一正式离线入口。
